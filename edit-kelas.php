...
if(isset($_POST['submit'])):	
	...
	if(empty($errors)):
		...
		$update = mysqli_query($koneksi,"UPDATE kelas SET kelas = '$kelas' WHERE id_kelas = '$id_kelas'");
		if($update) {
			redirect_to('list-kelas.php?status=sukses-edit');
		}else{
...
<form action="edit-kelas.php?id=<?php echo $id_kelas; ?>" method="post">
	<div class="card shadow mb-4">
		<div class="card-header py-3">
			<h6 class="m-0 font-weight-bold text-danger"><i class="fas fa-fw fa-edit"></i> Edit Data Kelas</h6>
		</div>
		<?php
		if(!$id_kelas) {
		?>
		...
		<div class="card-body">
			<div class="row">
				<div class="form-group col-md-12">
					...
					<input autocomplete="off" type="text" name="kelas" required value="<?php echo $d['kelas']; ?>" class="form-control"/>
				</div>
			</div>
		</div>
